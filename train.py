...
def main(cfg):
    ...
    if cfg.visualize:
        # Visualize
        # model_wts = torch.load(os.path.join(log_dir, 'checkpoint_best.pth'))
        model_wts = torch.load('imagenet/LTCC_PRCC58.6.pth')
        model.load_state_dict(model_wts['state_dict'])
...
def train(model, model_face, train_loader, test_loader, loss_fn, loss_face_fn, optimizer, scheduler, num_epochs, num_query, log_dir):
    ...
    for epoch in range(num_epochs):
        ...
        for ii, (img, target, path, mask, face) in enumerate(
            ...
            #print(score.size(), feat.size(), score_face.size(), feat_face.size())
            #torch.Size([80, 150])
            #torch.Size([80, 2048])
            #torch.Size([40, 150])
            #torch.Size([40, 512])
            ...
            loss = loss_fn(score, feat, score_face, feat_face, target_c, target)
